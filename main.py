...
for index in range(len(details)):
    ...
    if details[index][1] in selection:
        ...
        # can make content neat and tidy, easy to see
...
time_selection = st.select_slider(label = "Select a range of finishing time", options = all_time, value = (min(all_time), max(all_time)))
